     ...
     if (x !== null && (typeof x === 'object' || typeof x === 'function')) {
         ...
         try {
             ...
             if (typeof then === 'function') {
                 // 如果then是个函数 那么就调用then 并且把成功回调和失败回调传进去
                 // 如果x是一个promise 并且最终状态时成功，那么就会执行成功的回调
                 // 如果失败就会执行失败的回调如果失败了，就把失败的原因reject出去
                 // 做为promise2的失败原因，如果成功了那么成功的value时y，这个y有可能仍然是promise
                 // 所以需要递归调用resolvePromise这个方法 直达返回值不是一个promise
                 then.call(x, y => {
                     if (called) return;
                     called = true;
